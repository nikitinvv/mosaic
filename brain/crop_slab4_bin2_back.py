...
fname = '/data/2022-11-Nikitin/slab4/mosaic_rec/mosaic_bin2_rec_bin_cropped_ward/recon/r_00000.tiff'
fnameout = '/data/2022-11-Nikitin/slab4/mosaic_rec/mosaic_bin2_rec_bin_cropped_ward_back/recon'

data = dxchange.read_tiff_stack(fname,ind=range(0,53))[:]


os.system('rm -rf /data/2022-11-Nikitin/slab4/mosaic_rec/mosaic_bin2_rec_bin_cropped_ward_back/*')
mmin = -0.0029269056 
mmax = 0.0068289298

data = np.pad(data,((16,16),(0,0),(0,0)))
data = data.swapaxes(0,1)
data = ward_parallel(data,41,mmin,mmax,conv=False)
data = data.swapaxes(0,1)
data = data.swapaxes(0,2).swapaxes(1,2)
data = ward_parallel(data,-20,mmin,mmax,conv=False)
data=data[::-1]
